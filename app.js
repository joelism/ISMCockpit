...
async function driveSignIn() {
  ...
  try {
    ...
    if (!driveState.tokenClient) {
      driveState.tokenClient = oauth2.initTokenClient({
        ...
        callback: async (resp) => {
          try {
            if (!resp || resp.error) {
              ...
              alert(
                "Google Drive Login fehlgeschlagen.\n\n" +
                  "Fehler: " +
                  (resp && (resp.error_description || resp.error)
                    ? (resp.error_description || resp.error)
                    : "Unbekannt") +
                  "\n\n" +
                  "Hinweis: Bei GitHub Pages ist häufig die OAuth-Konfiguration schuld (Authorized JavaScript origins)."
              );
              return;
            }

            driveState.accessToken = resp.access_token || null;
            ...
            console.error(err);
            alert("Google Drive Login: Unerwarteter Fehler.\n" + (err.message || String(err)));
          } finally {
            driveState.loading = false;
          }
        },
        error_callback: (err) => {
          ...
          alert(
            "Google Drive Login fehlgeschlagen.\n\n" +
              (err && (err.message || err.type) ? (err.message || err.type) : "Unbekannter Fehler") +
              "\n\n" +
              "Tipp: Prüfe Popup-Blocker und in Google Cloud Console die 'Authorized JavaScript origins' (GitHub Pages URL)."
          );
        }
      });
    }
    ...
    const originalCb = driveState.tokenClient.callback;
    driveState.tokenClient.callback = async (resp) => {
      callbackFired = true;
      return originalCb(resp);
    };
    ...
    setTimeout(() => {
      if (!callbackFired && driveState.loading) {
        driveState.loading = false;
        alert(
          "Google Drive Login: Es ist kein Login-Fenster erschienen.\n\n" +
            "Bitte prüfe:\n" +
            "• Popup-Blocker (für GitHub Pages erlauben)\n" +
            "• In Google Cloud Console: OAuth Client → 'Authorized JavaScript origins' muss deine GitHub Pages URL enthalten.\n" +
    ...
    alert(
      "Anmeldung bei Google Drive ist fehlgeschlagen:\n" +
        (e && (e.error || e.details || e.message) ? (e.error || e.details || e.message) : String(e))
    );
  }
}
