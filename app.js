...
// Warm-up Drive (loads GAPI + GIS without needing a button gesture).
// This avoids popup-blocking caused by awaiting script loads inside the click handler.
function warmupDrive() {
  if (driveState._warmupPromise) return driveState._warmupPromise;
  driveState._warmupPromise = (async () => {
    try {
      await initDriveClient();
      await ensureGisLoaded();
      driveState._warmupOk = true;
    } catch (e) {
      driveState._warmupOk = false;
      driveState._warmupErr = e;
      console.error(e);
    }
  })();
  return driveState._warmupPromise;
}


async function driveSignIn() {
  // IMPORTANT: do not await script-loading here, otherwise the browser may block the OAuth popup.
  try {
    // If not ready yet, start warmup and ask user to click again.
    if (!driveState.clientInited || !(window.google && window.google.accounts && window.google.accounts.oauth2)) {
      warmupDrive();
      alert("Google Drive wird initialisiert. Bitte in 2-3 Sekunden nochmals auf \"Mit Google Drive verbinden\" klicken.\n\nFalls gar nichts passiert: Popup-Blocker prüfen und die GitHub-Pages Domain als OAuth-Origin freigeben.");
      return;
    }
      ...
      driveState.tokenClient = oauth2.initTokenClient({
        ...
        callback: async (resp) => {
          try {
            if (resp && resp.error) {
              console.error(resp);
              alert("Google Drive Anmeldung fehlgeschlagen: " + (resp.error_description || resp.error));
              return;
            }
            ...
            console.error(e);
            alert("Google Drive Anmeldung: interner Fehler\n" + (e.message || e.toString()));
          }
        }
      });
    }
...
async function driveSignOut() {
  try {
    ...
    // tokenClient may keep internal state; keep it but user will be forced to consent again if needed
    updateDriveStatus && updateDriveStatus();
  }
}
...
function renderTabContacts(selected, body) {
  ...
  const maidenNameInput = document.createElement("input");
  maidenNameInput.className = "db-input";
  maidenNameInput.placeholder = "Geburtsname (optional)";

  const civilStatusInput = document.createElement("input");
  civilStatusInput.className = "db-input";
  civilStatusInput.placeholder = "Zivilstand (optional)";
...
function renderGlobalDatabase() {
  ...
  const photoInput = document.createElement("input");
  photoInput.className = "db-input";
  photoInput.placeholder = "Foto-URL";

  const notesInput = document.createElement("textarea");
  notesInput.className = "db-input";
  notesInput.placeholder = "Weitere Infos / Notizen";
  notesInput.rows = 3;

  const saveBtn = document.createElement("button");
  saveBtn.className = "db-btn-ghost db-primary";
  saveBtn.textContent = "Person speichern";
  ...
  saveBtn.addEventListener("click", () => {
    ...
    const contactLike = {
      ...
      dob: dobInput.value || "",
      gender: genderSel.value || "",
      heightCm: heightInput.value ? String(heightInput.value) : "",
      nationality: (nationalityInput.value || "").trim(),
      elnr: (phoneInput.value || "").trim(),
      address: (addrInput.value || "").trim(),
      email: (mailInput.value || "").trim(),
    ...
    // übernimmt neue Person in globale DB
  ...
  form.append(
    ...
    labelWrap("Ermittlungsgrund", reasonInput),
    labelWrap("Instagram @", instaInput),
    labelWrap("Snapchat Benutzername", snapInput),
    labelWrap("TikTok Benutzername", tiktokInput),
    labelWrap("Haarfarbe", hairInput),
    labelWrap("Augenfarbe", eyeInput),
    labelWrap("Statur / besondere Merkmale", buildInput),
    labelWrap("Ausweisdaten", idDocInput),
  ...
  // Wichtig: Global-DB hat 7 Spalten -> sonst bricht das Grid auseinander.
  const colDef = "1.2fr .9fr 1.2fr 1fr 1.8fr .6fr 1.2fr";

  const head = document.createElement("div");
  head.className = "db-row db-row-head";
  head.style.gridTemplateColumns = colDef;
  head.innerHTML =
    `<div>Name</div><div>Geburt</div><div>Nationalität</div><div>Telefon</div><div>Ermittlungsgrund</div><div>Fälle</div><div>Aktionen</div>`;
  table.appendChild(head);
  ...
  if (!list.length) {
    ...
    list.forEach(p => {
      ...
      showBtn.textContent = "Details";
      showBtn.addEventListener("click", () => {
        openPersonEditDialog(p); // Option A: Bearbeiten/Löschen im Detaildialog
      });
...
(function init() {
  initTheme();
  // Prepare Google Drive libraries early (no popup).
  try { warmupDrive(); } catch {}
  initGlobalUi();
