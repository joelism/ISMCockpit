...
// Konfiguration – hier hast du deinen eigenen API-Key und Client-ID eingetragen
const GD = {
...
function gapiLoadClientAuth() {
  return new Promise((resolve, reject) => {
    window.gapi.load("client:auth2", {
      callback: resolve,
      onerror: () =>
        reject(new Error("Google Client konnte nicht initialisiert werden."))
    });
  });
}
...
async function initDriveClient() {
  ...
  await gapiLoadClientAuth();
  await gapi.client.init({
    apiKey: GD.API_KEY,
    clientId: GD.CLIENT_ID,
...
async function driveSignIn() {
  try {
    ...
    console.error("Drive signIn error:", e);
    alert(
...
async function driveSignOut() {
  try {
    ...
    console.error("Drive signOut error:", e);
  }
}
...
async function driveLoadFiles() {
  ...
  try {
    ...
    alert(
      "Dateien konnten nicht aus Google Drive geladen werden:\n" +
        (e.error || e.details || e.message || e.toString())
    );
  } finally {
    driveState.loading = false;
...
function renderTabDrive(selected, body) {
  ...
  head.innerHTML = `
    ...
    <div class="db-section-meta">
      Anmeldung mit eigenem Google-Konto, Anzeige der letzten Dateien (nur Metadaten).
    </div>
  `;
  ...
  function updateStatus() {
    if (driveState.loading) {
      status.textContent = "Status: Lade Daten von Google Drive …";
    } else if (driveState.signedIn) {
      status.textContent = "Status: Angemeldet – Dateien werden angezeigt.";
    } else {
  ...
  signInBtn.className = "primary db-primary";
  signInBtn.textContent = "Mit Google Drive verbinden";

  const signOutBtn = document.createElement("button");
  signOutBtn.className = "db-btn-ghost";
  ...
  btnRow.append(signInBtn, signOutBtn);
  ...
  function renderFileTable() {
    ...
    if (!files.length) {
      ...
      p.style.opacity = ".8";
      p.textContent =
        "Es wurden keine Dateien gefunden (oder keine Berechtigung).";
      tableWrap.appendChild(p);
      return;
    }
    ...
    headRow.innerHTML =
      "<div>Name</div><div>Typ</div><div>Geändert</div><div>Link</div><div></div>";
    ...
    files.forEach(f => {
      ...
      typeCell.textContent = f.mimeType || "";
      ...
      row.append(nameCell, typeCell, modCell, linkCell, dummy);
      table.appendChild(row);
    });
  ...
  // Beim Öffnen des Tabs versuchen wir, bestehenden Login zu erkennen
  (async () => {
    try {
      await initDriveClient();
      const auth = window.gapi.auth2.getAuthInstance();
      ...
      console.warn("Drive-Client Auto-Init fehlgeschlagen:", e);
    } finally {
      updateStatus();
      renderFileTable();
    }
