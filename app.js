/* ISM Cockpit – Cases, Login, and modules */
...
function doLogin(){
  ...
  if (!ok){ $('#loginError').style.display='block'; return; }
...
function renderDashboard(){
  ...
  casesCard.className = 'card';
  const ccount = state.cases.length;
  casesCard.innerHTML = `<h2>📁 Fälle</h2>
    <p>${ccount} Fall/Fälle</p>
    <div class="btn-row"><a class="btn" href="#/cases">Öffnen</a></div>`;

  const notesCard = document.createElement('div');
  notesCard.className = 'card';
  notesCard.innerHTML = `<h2>📝 Notizen</h2>
    <p>${state.notes.length} Notiz(en)</p>
    <div class="btn-row"><a class="btn" href="#/notes">Öffnen</a></div>`;

  const tasksCard = document.createElement('div');
  tasksCard.className = 'card';
  const openTasks = state.tasks.filter(t=>!t.done).length;
  tasksCard.innerHTML = `<h2>✅ Aufgaben</h2>
    <p>${openTasks} offen, ${state.tasks.length} gesamt</p>
    <div class="btn-row"><a class="btn" href="#/tasks">Öffnen</a></div>`;

  wrap.append(casesCard, notesCard, tasksCard);
...
function renderCases(){
  ...
  // Right pane: case detail (if any selected)
  ...
  if (selected){
    ...
    // Click handlers inside right pane
...
function exportFolder(fall, folder){
  const data = JSON.stringify({
    case: fall.title, folder: folder.name,
    exportedAt: new Date().toISOString(),
    docs: folder.docs
  }, null, 2);
  const blob = new Blob([data], { type:'application/json' });
...
/* NOTES/TASKS/LINKS (unchanged core) */
function renderNotes(){
  ...
  const q = state.search.trim().toLowerCase();
  state.notes
    .slice().sort((a,b)=>b.updated-a.updated)
...
function renderTasks(){
  ...
  const list = document.createElement('div'); list.className='grid';
  state.tasks
    .slice().sort((a,b)=>a.done - b.done || b.updated - a.updated)
...
function renderHelp(){
  ...
  card.innerHTML = `<h2>🛈 ISM Helpcenter</h2><p>Inhalt folgt. (Platzhalter)</p>`;
  view.append(card);
}
function renderMy(){
  const view = $('#view');
  const card = document.createElement('div'); card.className='card';
  card.innerHTML = `<h2>🪪 My ISM</h2><p>Ihr Beamtenausweis erscheint in Kürze hier.</p>`;
  view.append(card);
}
...
window.addEventListener('beforeinstallprompt', (e) => {
  ...
  btn.onclick = async () => { deferredPrompt.prompt(); deferredPrompt = null; btn.hidden = true; };
});
...
/* Init */
initTheme();
if (!state.notes.length && !state.tasks.length && !state.links.length && !state.cases.length){
  state.cases = [ { id: uid(), title:'DEMO-CASE', created: now(), folders: [ { id: uid(), name:'Berichte', docs: [] } ] } ];
  save(keys.cases, state.cases);
}
syncRoute();
