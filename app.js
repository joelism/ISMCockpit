...
function openPersonReportWindow(person, sourceLabel) {
  ...
  if (!win) {
    alert("Pop-up Blocker verhindert das Öffnen des Personenberichts.");
    return;
  }
  const title = `Personenbericht - ${person.name || ""}`;
  const html = buildPersonReportHtml(person, sourceLabel);
...
function renderGlobalDatabase() {
  ...
  head.className = "db-row db-row-head";
  head.innerHTML =
    `<div>Name</div><div>Geburt</div><div>Nationalität</div><div>Telefon</div><div>Fälle</div>`;
  table.appendChild(head);
