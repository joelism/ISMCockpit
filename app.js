...
async function driveSignIn() {
  // Start interactive OAuth flow (Google Identity Services)
  // Important: handle errors INSIDE the callback; throwing there will not be caught by outer try/catch.
  ...
  try {
      ...
      driveState.tokenClient = oauth2.initTokenClient({
        ...
        callback: (resp) => {
          try {
            if (!resp || resp.error) {
              ...
              const msg =
                (resp && (resp.error_description || resp.error)) ?
                  ...
                  "Unbekannter Fehler";
              alert(
                "Anmeldung bei Google Drive ist fehlgeschlagen:\n" +
                msg +
                "\n\nTipp: Prüfe Popup-Blocker und ob deine GitHub-Pages Domain im Google Cloud OAuth-Client als 'Authorized JavaScript origin' eingetragen ist."
              );
            ...
            // Load root files so the UI can show a successful connection
            Promise.resolve(driveLoadFiles())
              .catch((e) => {
                console.error(e);
                alert("Anmeldung war erfolgreich, aber Dateien konnten nicht geladen werden.");
              });
          } finally {
    ...
    // Keep loading=true until callback finishes
    return true;
  } catch (e) {
    console.error(e);
    driveState.loading = false;
...
function renderSettings() {
  ...
  driveConnectBtn.addEventListener("click", async () => {
    // Visual feedback immediately (otherwise it feels like "nothing happens")
    driveStatus.textContent = "Status: Öffne Google Login … (Popup ggf. erlauben)";
    try {
      await driveSignIn();
    } finally {
      // Token arrives asynchronously via callback; update shortly after
      setTimeout(updateDriveStatus, 800);
    }
  });
