...
function renderSettings() {
  ...
  driveConnectBtn.addEventListener("click", async () => {
    try {
      console.log("[ISM] Starte Google-Drive-Anmeldung…");
      await driveSignIn();
    } catch (e) {
      console.error(e);
      alert("Fehler bei der Anmeldung bei Google Drive. Details siehe Konsole.");
    } finally {
      updateDriveStatus();
    }
  });
...
function renderGlobalDatabase() {
  ...
  // Top-Leiste: Suche + Aktualisieren + Neue Person
  ...
  const searchInput = document.createElement("input");
  searchInput.className = "db-input";
  searchInput.placeholder = "Suche nach Name / Geburtsdatum / Telefon / Nationalität / Ermittlungsgrund";
  searchInput.value = state.search || "";
  searchInput.addEventListener("input", e => {
    state.search = e.target.value;
    render("/global-db");
  ...
  syncBtn.addEventListener("click", async () => {
    await driveSyncGlobalPeople();
    // nach Sync neu laden, falls lokale Daten gemerged wurden
    render("/global-db");
  });

  const newBtn = document.createElement("button");
  newBtn.className = "primary db-primary";
  newBtn.textContent = "+ Neue Person (ohne Fall) erfassen";

  controls.append(searchInput, syncBtn, newBtn);
  card.appendChild(controls);

  // Formular zum Erfassen einer neuen Person (unter dem Button ein-/ausblendbar)
  ...
  const addrInput = document.createElement("input");
  addrInput.className = "db-input";
  addrInput.placeholder = "Adresse / Wohnort";
  ...
  const idDocInput = document.createElement("input");
  idDocInput.className = "db-input";
  idDocInput.placeholder = "Ausweisdaten";
  ...
  form.append(
    ...
    labelWrap("Nationalität", nationalityInput),
    labelWrap("Telefonnummer", phoneInput),
    labelWrap("Adresse / Wohnort", addrInput),
    labelWrap("E-Mail", mailInput),
    labelWrap("Ermittlungsgrund", reasonInput),
    labelWrap("Haarfarbe", hairInput),
    labelWrap("Augenfarbe", eyeInput),
    labelWrap("Statur / Merkmale", buildInput),
    labelWrap("Ausweisdaten", idDocInput),
    labelWrap("Foto-URL", photoInput),
    labelWrap("Weitere Infos", notesInput),
    saveBtn
  ...
  const head = document.createElement("div");
  head.className = "db-row db-row-head";
  head.innerHTML = "<div>Name</div><div>Geburtsdatum</div><div>Ermittlungsgrund</div><div>Adresse / Land</div><div>Fälle</div><div>Aktionen</div>";
  table.appendChild(head);

  const q = (state.search || "").toLowerCase();
  const list = (state.people || []).slice().filter(p => {
    if (!q) return true;
    const haystack = [
      p.name || "",
      p.dob || "",
  ...
  if (!list.length) {
    const row = document.createElement("div");
    row.className = "db-row";
    row.innerHTML = '<div class="db-cell-empty" colspan="6">Noch keine Personen erfasst.</div>';
    table.appendChild(row);
  } else {
    list.forEach(p => {
      const row = document.createElement("div");
      ...
      addrCell.textContent = p.address || p.nationality || "–";

      const casesCell = document.createElement("div");
      const casesTxt = p.cases && p.cases.length
      ...
      function openEditDialog() {
        ...
        f.className = "grid db-form-grid";

        const eName = document.createElement("input");
        eName.className = "db-input";
        eName.value = p.name || "";

        const eDob = document.createElement("input");
        eDob.type = "date";
        eDob.className = "db-input";
        eDob.value = p.dob || "";

        const eGender = document.createElement("select");
        eGender.className = "db-input db-select";
        [
          ["unbekannt", "Unbekannt"],
          ["männlich", "Männlich"],
          ["weiblich", "Weiblich"],
          ...
          o.value = v;
          o.textContent = l;
          eGender.appendChild(o);
        });
        eGender.value = p.gender || "unbekannt";

        const eHeight = document.createElement("input");
        eHeight.type = "number";
        eHeight.className = "db-input";
        eHeight.value = p.heightCm || "";

        const eNat = document.createElement("input");
        eNat.className = "db-input";
        eNat.value = p.nationality || "";

        const ePhone = document.createElement("input");
        ePhone.className = "db-input";
        ePhone.value = p.elnr || "";

        const eAddr = document.createElement("input");
        eAddr.className = "db-input";
        eAddr.value = p.address || "";

        const eMail = document.createElement("input");
        eMail.type = "email";
        eMail.className = "db-input";
        eMail.value = p.email || "";

        const eReason = document.createElement("input");
        eReason.className = "db-input";
        eReason.value = p.reason || "";

        const eHair = document.createElement("input");
        eHair.className = "db-input";
        eHair.value = p.hairColor || "";

        const eEye = document.createElement("input");
        eEye.className = "db-input";
        eEye.value = p.eyeColor || "";

        const eBuild = document.createElement("input");
        eBuild.className = "db-input";
        eBuild.value = p.build || "";

        const eIdDoc = document.createElement("input");
        eIdDoc.className = "db-input";
        eIdDoc.value = p.idDoc || "";

        const ePhoto = document.createElement("input");
        ePhoto.className = "db-input";
        ePhoto.value = p.photoUrl || "";

        const eNotes = document.createElement("textarea");
        eNotes.className = "db-input";
        eNotes.rows = 2;
        eNotes.value = p.notes || "";

        f.append(
          labelWrap("Name", eName),
          labelWrap("Geburtsdatum", eDob),
          labelWrap("Geschlecht", eGender),
          labelWrap("Körpergrösse (cm)", eHeight),
          labelWrap("Nationalität", eNat),
          labelWrap("Telefonnummer", ePhone),
          labelWrap("Adresse / Wohnort", eAddr),
          labelWrap("E-Mail", eMail),
          labelWrap("Ermittlungsgrund", eReason),
          labelWrap("Haarfarbe", eHair),
          labelWrap("Augenfarbe", eEye),
          labelWrap("Statur / Merkmale", eBuild),
          labelWrap("Ausweisdaten", eIdDoc),
          labelWrap("Foto-URL", ePhoto),
          labelWrap("Weitere Infos", eNotes)
        );
        box.appendChild(f);
        ...
        deleteBtn.addEventListener("click", () => {
          const msg =
            "Das Löschen dieser Person ohne entsprechende Berechtigung kann zu internen Untersuchungen " +
            "sowie rechtlichen Schwierigkeiten durch den ISM führen.\n\n" +
        ...
        saveEditBtn.className = "primary db-primary";
        saveEditBtn.addEventListener("click", () => {
          ...
          p.heightCm = eHeight.value.trim();
          p.nationality = eNat.value.trim();
          p.elnr = ePhone.value.trim();
          p.address = eAddr.value.trim();
          p.email = eMail.value.trim();
          p.reason = eReason.value.trim();
          p.hairColor = eHair.value.trim();
          p.eyeColor = eEye.value.trim();
          p.build = eBuild.value.trim();
          p.idDoc = eIdDoc.value.trim();
          p.photoUrl = ePhoto.value.trim();
          p.notes = eNotes.value.trim();
          p.updated = now();
          save(KEYS.people, state.people);
          document.body.removeChild(overlay);
          render("/global-db");
        ...
        buttons.append(cancelBtn, deleteBtn, saveEditBtn);
      ...
      actions.append(showBtn, editBtn, reportBtn);
      actionsCell.appendChild(actions);

      row.append(nameCell, dobCell, reasonCell, addrCell, casesCell, actionsCell);
      table.appendChild(row);
    });
  }
