...
// Konfiguration für Google Drive – HIER DEINE DATEN EINTRAGEN
const GD = {
  API_KEY: "HIER_DEIN_API_KEY",
  CLIENT_ID: "HIER_DEINE_CLIENT_ID.apps.googleusercontent.com",
...
async function driveSignIn() {
  try {
    ...
    console.error(e);
    alert("Anmeldung bei Google Drive ist fehlgeschlagen. Bitte API-Key und Client-ID prüfen.");
  } finally {
...
function renderTabDrive(selected, body) {
  ...
  function renderFileTable() {
    ...
    if (!files.length) {
      ...
      p.style.fontSize = ".85rem";
      p.style.opacity = ".8";
      p.textContent = "Es wurden keine Dateien gefunden (oder keine Berechtigung).";
      tableWrap.appendChild(p);
      return;
    }
    ...
    headRow.innerHTML = "<div>Name</div><div>Typ</div><div>Geändert</div><div>Link</div><div></div>";
    ...
    files.forEach(f => {
      ...
      row.innerHTML = `
        ...
        <div>${escapeHtml(f.mimeType || "")}</div>
        <div>${escapeHtml(f.modifiedTime || "")}</div>
      `;
      row.append(linkCell, dummy);
      table.appendChild(row);
    });
  ...
  (async () => {
    try {
      ...
      console.warn("Drive-Client konnte nicht automatisch initialisiert werden:", e);
    } finally {
      updateStatus();
      renderFileTable();
    }
