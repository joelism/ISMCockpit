...
function renderTabReports(selected, body) {
  ...
  dateInput.type = "text";
  dateInput.placeholder = "YYYY-MM-DD oder YYYY";
...
// --- Drive Auth Prep (wichtig für Popup-Login) ---
// Problem: Wenn vor requestAccessToken() erst async geladen/init gemacht wird,
// geht der "User-Gesture"-Kontext verloren → Popup öffnet nicht.
// Lösung: Auth/Client bereits vorab vorbereiten, und beim Klick nur noch requestAccessToken auslösen.

let _driveAuthPrepPromise = null;

function drivePrepareAuth() {
  if (_driveAuthPrepPromise) return _driveAuthPrepPromise;
  _driveAuthPrepPromise = (async () => {
...
" +
                  (resp.error || resp.details || JSON.stringify(resp))
              );
            ...
            driveState.accessToken = resp && resp.access_token ? resp.access_token : null;
            if (window.gapi && window.gapi.client && driveState.accessToken) {
              window.gapi.client.setToken({ access_token: driveState.accessToken });
            }
            driveState.signedIn = !!driveState.accessToken;
            await driveLoadFiles();
          } catch (e) {
            console.error(e);
            alert("Fehler nach Google-Login:
" + (e.message || e.toString()));
  ...
  return _driveAuthPrepPromise;
}

function _drivePopupLikelyBlockedHint() {
  // Wenn nach einem Klick keine Callback-Reaktion kommt, ist häufig ein Popup-Blocker oder OAuth-Origin Problem schuld.
  setTimeout(() => {
    if (!driveState.signedIn && !driveState.loading) {
      // Nur Hinweis – kein harter Fehler.
      console.warn("[Drive] Kein Login-Popup / keine Antwort erhalten. Popup-Blocker oder OAuth-Origin prüfen.");
    }
  }, 2500);
}

// Wichtig: driveSignIn ist absichtlich NICHT "await-lastig" vor requestAccessToken()
// Der Click-Handler ruft drivePrepareAuth() frühzeitig auf (beim App-Start), damit der Popup-Flow klappt.
function driveSignIn() {
  driveState.loading = true;

  // Wenn bereits vorbereitet → direkt requestAccessToken (im selben Tick wie Klick)
  if (driveState.tokenClient) {
    try {
      driveState.tokenClient.requestAccessToken({
        prompt: driveState.signedIn ? "" : "consent"
      });
      _drivePopupLikelyBlockedHint();
    } catch (e) {
      console.error(e);
      alert("Google Login konnte nicht gestartet werden:
" + (e.message || e.toString()));
    } finally {
  ...
  // Fallback: Vorbereitung läuft noch – Hinweis und beim nächsten Klick klappt es sicher.
  drivePrepareAuth()
    .then(() => {
      // Nicht automatisch requestAccessToken hier (wäre evtl. nicht mehr im User-Gesture-Kontext).
      alert("Google Drive ist bereit. Bitte klicke nochmals auf „Mit Google Drive verbinden“.");
    })
    .catch((e) => {
      console.error(e);
      alert("Google Drive konnte nicht initialisiert werden:
" + (e.message || e.toString()));
    })
    .finally(() => {
      driveState.loading = false;
...
async function driveSignOut() {
  try {
    ...
    const token =
      ...
      (window.gapi && window.gapi.client && window.gapi.client.getToken && window.gapi.client.getToken() && window.gapi.client.getToken().access_token);
...
function renderTabContacts(selected, body) {
  ...
  dobInput.type = "text";
  dobInput.placeholder = "Geburtsdatum (YYYY-MM-DD) oder Jahrgang (YYYY)";
  ...
  birthNameInput.className = "db-input";
  birthNameInput.placeholder = "Geburtsname";

  const civilStatusInput = document.createElement("input");
  civilStatusInput.className = "db-input";
  civilStatusInput.placeholder = "Zivilstand (z.B. ledig, verheiratet)";
  ...
  saveBtn.addEventListener("click", () => {
    ...
    const contact = {
      ...
      civilStatus: civilStatusInput.value.trim(),
  ...
  form.append(
    ...
    labelWrap("Geburtsname", birthNameInput),
    labelWrap("Zivilstand", civilStatusInput),
...
function renderSettings() {
  ...
  driveConnectBtn.addEventListener("click", () => {
    driveSignIn();
    // Status kurz danach aktualisieren (Callback setzt signedIn + lädt Dateien)
    setTimeout(updateDriveStatus, 500);
    setTimeout(updateDriveStatus, 2000);
  });
...
function openPersonEditDialog(person) {
  ...
  function makeInput(label, initial, type = "text") {
  ...
  const fPhone   = makeInput("Telefonnummer", person.elnr || person.phone);
  ...
  const fNotes   = makeInput("Weitere Infos / Notizen", person.notes, "textarea");
  const fInsta   = makeInput("Instagram @", person.instagram);
  const fSnap    = makeInput("Snapchat Benutzername", person.snapchat);
  const fTikTok  = makeInput("TikTok Benutzername", person.tiktok);
  ...
  saveBtn.addEventListener("click", () => {
    ...
    person.birthName   = fBirthName.input.value.trim();
    person.civilStatus  = fCivilStatus.input.value.trim();
    ...
    person.instagram   = fInsta.input.value.trim();
    person.snapchat    = fSnap.input.value.trim();
    person.tiktok      = fTikTok.input.value.trim();
    person.updated     = now();
    save(KEYS.people, state.people);
    document.body.removeChild(overlay);
    render("/global-db");
...
function initGlobalUi() {
  ...
  // Google Drive: frühzeitig vorbereiten, damit der Popup-Login beim Klick nicht blockiert wird.
  // (Popup öffnet oft nicht, wenn vor requestAccessToken() erst async geladen wird.)
  try { drivePrepareAuth(); } catch (_) {}
